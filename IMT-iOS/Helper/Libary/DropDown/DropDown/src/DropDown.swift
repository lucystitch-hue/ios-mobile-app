...
public final class DropDown: UIView {
    ...
    /// The list dont allow selected
    public var blackListSelected: [Int] = []
    
    public var focusAt: Int = -1 {
        didSet {
            self.setFocusAt(focusAt)
        }
    }
	...
	*/
	...
	//MARK: Constraints
	fileprivate var heightConstraintDropDown: NSLayoutConstraint!
	fileprivate var widthConstraintDropDown: NSLayoutConstraint!
...
extension DropDown {
	...
	public override func updateConstraints() {
		...
		widthConstraintDropDown.constant = layout.width
		heightConstraintDropDown.constant = layout.visibleHeight
	...
	fileprivate func setupConstraints() {
        ...
        widthConstraintDropDown = NSLayoutConstraint(
			item: tableViewContainer,
			attribute: .width,
			relatedBy: .equal,
			toItem: nil,
			...
			multiplier: 1,
			constant: 0)
		tableViewContainer.addConstraint(widthConstraintDropDown)

		heightConstraintDropDown = NSLayoutConstraint(
			item: tableViewContainer,
			attribute: .height,
			relatedBy: .equal,
			toItem: nil,
			...
			multiplier: 1,
			constant: 0)
		tableViewContainer.addConstraint(heightConstraintDropDown)
...
extension DropDown {
	...
	*/
    ...
    ///Handle logic and user interface when deselect
    public func deselectAllRow() {
        setFocusAt(self.focusAt)
        deselectRows(at: selectedRowIndices)
        reloadAllComponents()
    }
...
extension DropDown: UITableViewDataSource, UITableViewDelegate {
	...
	public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let allowSelected = !blackListSelected.contains(selectedRowIndex)
        guard let cell = tableView.cellForRow(at: indexPath) as? DropDownCell else { return }
...
//MARK: Private
extension DropDown {
    private func setFocusAt(_ index: Int) {
        if(index > -1) {
